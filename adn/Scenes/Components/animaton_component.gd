...
@export var frontHorn:AnimatedSprite2D
...
func handle_horizontal_flip(move_dir: float)->void:
	...
	frontHorn.flip_h = false if move_dir > 0 else true
	backHorn.flip_h = false if move_dir > 0 else true
	eye.flip_h = false if move_dir > 0 else true
	
func handle_move_animaton(move_direcction:float)->void:
	handle_horizontal_flip(move_direcction)
	if move_direcction != 0:
		sprite.play("default")
		frontHorn.play("default")
		backHorn.play("default")
		eye.play("default")
	else:
		sprite.play("idle")
		frontHorn.play("idle")
...
func handle_jump_animation(is_jumping:bool, is_falling:bool)->void:
	if is_jumping:
		sprite.play("jumpUp")
		frontHorn.play("jumpUp")
		backHorn.play("jumpUp")
		eye.play("jumpUp")
	elif is_falling:
		sprite.play("jumpDown")
		frontHorn.play("jumpDown")
